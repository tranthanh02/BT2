...
export default function ResultFood({ items }: { items: IItemResult[] }) {
  const router = useRouter();
  const handleNavigate = (food: IItemResult) => {
    router.push(`/detailfood/${food.food_slug}_${food.category_id}`);
  };
  return (
    <>
      <div className="mt-3 flex flex-row flex-wrap gap-3 ">
        {items.map((item: IItemResult) => (
          <div
          ...
          >
            ...
            <div className="group-hover:bg-slate-50 w-full h-[40%] pr-3  border border-solid">
              ...
              <div className=" flex items-center w-full text-sm border-t  border-beamin-50  h-[30%]">
                <span className="ml-3 ">{item.categories.category_name}</span>
              </div>
            </div>
          </div>
        ))}
