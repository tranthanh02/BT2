...
export default function HeaderNav() {
  ...
  const onSearch: SearchProps["onSearch"] = (value, _e, info) => {
    router.push(value ? `/search?name=${value}` : `/search`);
  };
  const navigation = () => {
  ...
  return (
    <div className="w-full h-fix bg-white flex flex-row fixed  py-3 gap-4 justify-items-center	justify-center z-50	">
      ...
      <div className="grow  flex flex-row items-center gap-9	 ">
        <Search
          className="w-3/4"
          placeholder="Món ăn bạn cần tìm kiếm ?"
      ...
      <div className="flex-none w-1/4  flex flex-row items-center  py-2">
        ...
        <Button
          onClick={() =>
            session ? router.push("/profile") : router.push("/login")
          }
          className="font-normal  leading-5 btn-home	"
          style={{
            fontSize: "18px",
        ...
        <Button
          // href="/cart"
          onClick={() => session && router.push("/cart")}
          type="text"
          htmlType="button"
          style={{
            fontSize: "20px",
